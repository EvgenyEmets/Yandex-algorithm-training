...
def intersection_point(p1, p2, p3, p4):
    ...
    if s1 == s2:
        ...
        x = (s1*b1 + s2*b2)/2
        y = (-1)*s1*x+b1
        if x <= max(p1[0], p2[0]) and x <= max(p3[0], p4[0]) and x >= min(p1[0], p2[0]) and x >= min(p3[0], p4[0]) and y <= max(p1[1], p2[1]) and y <= max(p3[1], p4[1]) and y >= min(p1[1], p2[1]) and y >= min(p3[1], p4[1]):
            return [x, y]
        else:
            return None
def between(p, r):
    return p[1] - p[0] >= r[2][1] - r[2][0] and p[1] - p[0] <= r[0][1] - r[0][0] and p[0]+p[1] >= r[3][0]+r[3][1] and p[0]+p[1] <= r[1][0]+r[1][1]
...
def intersection(c1, c2):
    ...
    for i in range(4):
        for j in range(4):
            tmp = intersection_point(c1[i], c1[(i+1)%4], c2[j], c2[(j+1)%4])
            if tmp != None:
    ...
    if len(corners) == 1:
        c = [[corners[0][0], corners[0][1]], [corners[0][0], corners[0][1]], [corners[0][0], corners[0][1]], [corners[0][0], corners[0][1]]]
